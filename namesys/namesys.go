	...
	go func() {
		...
		for {
			...
			case res, ok := <-resCh:
				...
				p, cacheTag, proof := res.value, res.cacheTag, res.proof
				...
				if len(segments) > 3 {
					...
					p, err = path.FromSegments("", strings.TrimRight(p.String(), "/"), segments[3])
					if err != nil {
						emitOnceResult(ctx, out, onceResult{value: p, cacheTag: cacheTag, proof: proof, ttl: res.ttl, err: err})
					}
				}

				emitOnceResult(ctx, out, onceResult{value: p, cacheTag: cacheTag, proof: proof, ttl: res.ttl, err: res.err})
			case <-ctx.Done():
				return
			}
		}
